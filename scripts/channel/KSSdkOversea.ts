...
const TAG_CHANNEL = TAG + "-KuaiShouOvs";
export default class KSSdk implements IXSdk {
    ...
    private getSystemInfo(): void {
        ...
        if (lang == "pt_BR") {
            this.currencyType = "BRL";
        } else if (lang == "id_ID") {
            this.currencyType = "IDR";
        }
    }
    ...
    private tryConsumeBalance(orderNo: string): Promise<any> {
        return new Promise((resolve, reject) => {
            HttpRequest.getInstance().request(RequestUrlOvs.KS_PAY_BALANCE, "POST", { "content-type": "application/json", "Authorization": "Bearer " + LocalStorage.getStringData("token") },
    ...
    private channelLogin(cbSuccess: any, cbFailure: any): void {
        ...
        ks.login({
            success: (res) => {
                if (res && res["code"]) {
                    this.xLog("登录成功", res);
                    this.loginParams["kuaishouOverseas"]["jsCode"] = res["code"];
    ...
    private parseAdParams(): void {
        ...
        if (!adParams["data"]["paymentStatus"]) {
            ...
            if (this.loginParams["kuaishouOverseas"]["jsCode"]) {
    ...
    private preOrder(productExternalId: string): Promise<any> {
        return new Promise((resolve, reject) => {
            HttpRequest.getInstance().request(RequestUrlOvs.KS_PRE_ORDER, "POST", { "content-type": "application/json", "Authorization": "Bearer " + LocalStorage.getStringData("token") },
                { productExternalId: productExternalId, platformType: this.systemInfo.platform.toUpperCase(), currencyType: this.currencyType }, (ret: boolean, res: object) => {
    ...
    async pay(params: object, callback: (phases: string, res: object) => void): Promise<void> {
        ...
        if (!this.loginParams["kuaishouOverseas"]["jsCode"] || !sessionPass) {
            this.xLog("用户未登录或登录已过期");
            this.xLog("code:" + this.loginParams["kuaishouOverseas"]["jsCode"], "sessionPass:" + sessionPass);
            if (!retryLogin) {
